...
void CQRCode_And_DecodeDlg::OnBnClickedRcode()
{
	...
	if(CodeOrDecode==0)//加密
	{
		...
		CBModeControl.AddString(_T("模式一"));//RSA加密+7z(LZMA2)压缩
		CBModeControl.AddString(_T("模式二"));//DES加密+7z(LZMA2)压缩
	...
	else if(CodeOrDecode==1)//解密
	{
		...
		CBModeControl.AddString(_T("模式一"));//RSA解密+7z(LZMA2)解压缩
		CBModeControl.AddString(_T("模式二"));//DES解密+7z(LZMA2)解压缩
		CBModeControl.SetCurSel(0);//设置第0项为显示的内容
...
void CQRCode_And_DecodeDlg::OnBnClickedBstart()
{
	...
	if(CodeOrDecode==0)//加密生成二维码
	{
		...
		if(nIndex==0)//模式一：RSA加密+7z(LZMA2)压缩
		{
			...
			strcpy(My7Z_Compress_path_name,".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件2).txt");
			...
			My7Z_File_Compress(RSA_Coded_path_name,My7Z_Compress_path_name);//7z(LZMA2)算法压缩
			for(int i=0;i<5;i++)
			{
				Sleep(200);
				//判断".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件2).txt"文件是否存在
				errno_t err_read;
	            FILE *fp_read = NULL;
	            err_read = fopen_s(&fp_read, ".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件2).txt", "rb");
	            if(err_read==0)
				{
					fclose(fp_read);
					break;
			...
			WriteFlag_Three_ASC(My7Z_Compress_path_name,Flag_path_name,'R','7','C');//RSA+7z+Code
			DeleteFile(MyCharToLPCTSTR(Origin_path_name));//删除将待生成二维码内容编辑框内的内容写入生成的文件
			DeleteFile(MyCharToLPCTSTR(RSA_Coded_path_name));//删除RSA加密后但未进行7z(LZMA2)压缩时产生的临时文件
			DeleteFile(MyCharToLPCTSTR(My7Z_Compress_path_name));//删除7z(LZMA2)后但未写入标记的临时文件
			CString number;
		...
		else if(nIndex==1)//模式二：DES加密+7z(LZMA2)压缩
		{
			SetDlgItemText(IDC_SSTATE,_T("正在生成二维码中..."));
			DeleteFile(_T("D:\\QRtemp.png"));
			char Origin_path_name[100];
			...
			My7Z_File_Compress(DES_Coded_path_name,My7Z_Compress_path_name);//7z(LZMA2)算法压缩
			for(int i=0;i<5;i++)
			{
				Sleep(200);
				//判断".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件2).txt"文件是否存在
			...
			WriteFlag_Three_ASC(My7Z_Compress_path_name,Flag_path_name,'D','7','C');//DES+7z+Code
			DeleteFile(MyCharToLPCTSTR(Origin_path_name));//删除将待生成二维码内容编辑框内的内容写入生成的文件
			DeleteFile(MyCharToLPCTSTR(DES_Coded_path_name));//删除DES加密后但未进行7z(LZMA2)压缩时产生的临时文件
			DeleteFile(MyCharToLPCTSTR(My7Z_Compress_path_name));//删除7z(LZMA2)后但未写入标记的临时文件
			CString number;
		    DataFromFileToNumber(MyCharToCString(Flag_path_name),&number);//将待生成二维码的数据复制到内存中
			MyQRCode(_T("D:\\QRtemp.png"),number);
	...
	else if(CodeOrDecode==1)//解密解析二维码
	{
		...
		if(nIndex==0)//(自动匹配模式)：自动匹配模式
		{
			...
			char Origin_path_name[100];
			char My7Z_Uncompress_path_name[100];
			...
			strcpy(My7Z_Uncompress_path_name,".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件1).txt");
			strcpy(RSAOrDES_Decoded_path_name,"D:\\QRtemp.txt");
			int flag_1=CheckFlag_Three_ASC(Origin_path_name,NoFLag_path_name,'R','7','C');//RSA+7z+Code
			if(flag_1==1)
			{
				My7Z_File_Uncompress(NoFLag_path_name,"tempfile(半加密临时文件1).txt");//7z(LZMA2)算法解压缩
				for(int i=0;i<5;i++)
			    {
				    Sleep(200);
				    //判断".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件1).txt"文件是否存在
				    errno_t err_read;
	                FILE *fp_read = NULL;
	                err_read = fopen_s(&fp_read, ".\\QRCode_And_Decode_Cache\\tempfile(半加密临时文件1).txt", "rb");
	                if(err_read==0)
				    {
					    fclose(fp_read);
					    break;
			    ...
			    RSA_File_Decode(My7Z_Uncompress_path_name,RSAOrDES_Decoded_path_name,217,29213);//RSA算法解密 d=217 n=29213 (p=131,q=223,n=29213,e=133,d=217)
			    DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
				DeleteFile(MyCharToLPCTSTR(NoFLag_path_name));//删除去除标志后但未进行RSA解密的临时文件
			    DeleteFile(MyCharToLPCTSTR(My7Z_Uncompress_path_name));//删除RSA解密后但未进行7z(LZMA2)压缩时产生的临时文件
			    RemoveDirectory(_T(".\\QRCode_And_Decode_Cache"));//删除产生的临时文件夹QRCode_And_Decode_Cache
			...
			{
				int flag_2=CheckFlag_Three_ASC(Origin_path_name,NoFLag_path_name,'D','7','C');//DES+7z+Code
			    if(flag_2==-2)
			    {
					DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
					RemoveDirectory(_T(".\\QRCode_And_Decode_Cache"));//删除产生的临时文件夹QRCode_And_Decode_Cache
			    ...
			    const char DES_Key[]="abcdefghijklmnopqrstuvwxyz";
			    des_decode_forallfile(My7Z_Uncompress_path_name,(const unsigned char*)DES_Key,strlen(DES_Key),RSAOrDES_Decoded_path_name);//DES算法解密
				DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
			    DeleteFile(MyCharToLPCTSTR(NoFLag_path_name));//删除去除标志后但未进行DES解密的临时文件
			    DeleteFile(MyCharToLPCTSTR(My7Z_Uncompress_path_name));//删除DES解密后但未进行7z(LZMA2)压缩时产生的临时文件
		...
		else if(nIndex==1)//模式一：RSA解密+7z(LZMA2)解压缩
		{
			SetDlgItemText(IDC_SSTATE,_T("正在解析二维码中..."));
			DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile1.txt"));
			DeleteFile(_T("D:\\QRtemp.txt"));
			...
			int flag=CheckFlag_Three_ASC(Origin_path_name,NoFLag_path_name,'R','7','C');//RSA+7z+Code
			if(flag==-1)
			{
				DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
				RemoveDirectory(_T(".\\QRCode_And_Decode_Cache"));//删除产生的临时文件夹QRCode_And_Decode_Cache
			...
			RSA_File_Decode(My7Z_Uncompress_path_name,RSA_Decoded_path_name,217,29213);//RSA算法解密 d=217 n=29213 (p=131,q=223,n=29213,e=133,d=217)
			DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
			DeleteFile(MyCharToLPCTSTR(NoFLag_path_name));//删除去除标志后但未进行RSA解密的临时文件
			DeleteFile(MyCharToLPCTSTR(My7Z_Uncompress_path_name));//删除RSA解密后但未进行7z(LZMA2)压缩时产生的临时文件
			RemoveDirectory(_T(".\\QRCode_And_Decode_Cache"));//删除产生的临时文件夹QRCode_And_Decode_Cache
		...
		else if(nIndex==2)//模式二：DES解密+7z(LZMA2)解压缩
		{
			SetDlgItemText(IDC_SSTATE,_T("正在解析二维码中..."));
			DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile1.txt"));
			DeleteFile(_T("D:\\QRtemp.txt"));
			...
			int flag=CheckFlag_Three_ASC(Origin_path_name,NoFLag_path_name,'D','7','C');//DES+7z+Code
			if(flag==-1)
			{
				DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
				RemoveDirectory(_T(".\\QRCode_And_Decode_Cache"));//删除产生的临时文件夹QRCode_And_Decode_Cache
			...
			const char DES_Key[]="abcdefghijklmnopqrstuvwxyz";
			des_decode_forallfile(My7Z_Uncompress_path_name,(const unsigned char*)DES_Key,strlen(DES_Key),DES_Decoded_path_name);//DES算法解密
			DeleteFile(_T(".\\QRCode_And_Decode_Cache\\tempfile2.txt"));
			DeleteFile(MyCharToLPCTSTR(NoFLag_path_name));//删除去除标志后但未进行DES解密的临时文件
			DeleteFile(MyCharToLPCTSTR(My7Z_Uncompress_path_name));//删除DES解密后但未进行7z(LZMA2)压缩时产生的临时文件
			RemoveDirectory(_T(".\\QRCode_And_Decode_Cache"));//删除产生的临时文件夹QRCode_And_Decode_Cache
...
void CQRCode_And_DecodeDlg::My7Z_File_Compress(const char* indatafilename,const char* outdatafilename)
{
	CString cstr=_T("");
	cstr.Format(_T("/c 7z a %s %s"),MyCharToCString(outdatafilename),MyCharToCString(indatafilename));
	ShellExecute(NULL, _T("open"), _T("cmd.exe"), cstr, NULL, SW_HIDE);
}


void CQRCode_And_DecodeDlg::My7Z_File_Uncompress(const char* indatafilename,const char* outdatafilename)
{
	CString cstr=_T("");
	cstr.Format(_T("/c 7z e -o%s %s %s"),_T("QRCode_And_Decode_Cache"),MyCharToCString(indatafilename),MyCharToCString(outdatafilename));
	ShellExecute(NULL, _T("open"), _T("cmd.exe"), cstr, NULL, SW_HIDE);
}
...
/*
bug说明：
原zlib压缩含有bug：生成与解析二维码信息量不宜过大，否则会在zlib压缩与解压缩阶段的动态分配与回收内存处出现异常，导致堆被破坏，此bug尚未修复。
现此程序已将原zlib压缩改为7z(LZMA2)压缩，原bug不再存在。
*/